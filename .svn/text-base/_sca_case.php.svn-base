...
//		header("location: sca_correspondencia_t_c.php?mensaje=$mensaje&dato=$dato");
	...
	case 24:
		...
		if ($conteo_sql_c==0)
			{
			$id_base_sigesp_t=1;
			$sql="insert into $tabla (base_db, id_base_db_t) values ('$base_sigesp_principal', '$id_base_sigesp_t')";
			$error="incluir Base Sigesp Principal";
			sql($sql, $error);
			
			$mensaje=1;
			$dato="Base Sigesp Incluida";

			$id_registro="$base_sigesp_principal";		
			...
			if (!(@mysql_num_rows ($sql_c) == 0))
				...
				while ($campo = mysql_fetch_array ($sql_c))
					...
					if ($id_base_sigesp_t==1)
						{
						if ($base_sigesp!=$base_sigesp_principal)
							{
							$where="where base_db='$base_sigesp' and id_base_db_t='1'";
							$id_registro=$base_sigesp_principal;
							$arreglo_campos_v = Array ('base_db');/*Nombre de los Campos en las Tabla Correspondiente de MYSQL*/
							$arreglo_variables_v= Array ('base_sigesp_principal'); /*Nombre de la Variables en el Formulario*/
							$error="actualizar Base Sigesp Principal";
		
							historial_m($tabla, $where, $id_registro, $arreglo_campos_v, $arreglo_variables_v, $error, $sca_id_u_activo, $ip_acceso_u, $fecha_larga);
							$mensaje=1;
							$dato="Base Sigesp Actualizada";
							}
						}
					}
				}
			}
		//Fin Base Sigesp Principal
	...
	case 27:
		...
		$variables="cedula=$cedula&nombre=$nombre&apellido=$apellido&telefono_movil=$telefono_movil&cod_ent=$cod_ent&id_visitante_t=$id_visitante_t&cod_procedencia=$cod_procedencia&id_asunto=$id_asunto&cod_emisor=$cod_emisor&emisor=$emisor&procedencia=$procedencia&autorizacion_nombre=$autorizacion_nombre&codper=$codper&cedper=$cedper&nomper=$nomper&apeper=$apeper&codemp=$codemp&minorguniadm=$minorguniadm&ofiuniadm=$ofiuniadm&uniuniadm=$uniuniadm&depuniadm=$depuniadm&prouniadm=$prouniadm&desuniadm=$desuniadm&regreso=1&observacion=$observacion";	
